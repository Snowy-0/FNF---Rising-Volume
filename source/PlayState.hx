...
class PlayState extends MusicBeatState
{
	...
	var subwayTrain:BGSprite;
	var subwaySound:FlxSound;
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'pico' | 'blammed' | 'philly-nice':
				...
				if(!ClientPrefs.lowQuality) {
					var streetBehind:BGSprite = new BGSprite('licotrain', -40, 50);
					add(streetBehind);
				}

				phillyTrain = new BGSprite('licotrain', 2000, 360);
				add(phillyTrain);
			...
			case 'pressure' | 'secondhand-thoughts' | 'good-condition' | 'livin-it' | 'badass' | 'dancing-days' | 'cold' | 'shudder' | 'clear-skies' | 'drowzy' | 'edge' | 'new-2me':
				defaultCamZoom = 0.6;
				curStage = 'subway';
				...
				subwaySound = new FlxSound().loadEmbedded(Paths.sound('train_passes'));
				CoolUtil.precacheSound('train_passes');
				FlxG.sound.list.add(subwaySound);
				...
				var ceilingglow:BGSprite = new BGSprite('ceiling_light_glow', -800, -405, 0.95, 0.98);
	...
	override public function update(elapsed:Float)
	{
		...
		switch (curStage)
		{
			...
			case 'subway':
				if (subwayMoving)
				{
					subwayFrameTiming += elapsed;

					if (subwayFrameTiming >= 1 / 24)
					{
						updateSubwayPos();
						subwayFrameTiming = 0;
					}
	...
	var subwayMoving:Bool = false;
	var subwayFrameTiming:Float = 0;

	var subwayCars:Int = 8;
	var subwayFinishing:Bool = false;
	var subwayCooldown:Int = 0;

	function subwayStart():Void
	{
		subwayMoving = true;
		if (!subwaySound.playing)
			subwaySound.play(true);
	}

	var startedMovingSubway:Bool = false;

	function updateSubwayPos():Void
	{
		if (subwaySound.time >= 4700)
		{
			startedMovingSubway = true;
			gf.playAnim('hairBlow');
			gf.specialAnim = true;
		}

		if (startedMovingSubway)
		{
			subwayTrain.x -= 400;

			if (subwayTrain.x < -2000 && !subwayFinishing)
			{
				subwayTrain.x = -1150;
				subwayCars -= 1;

				if (subwayCars <= 0)
					subwayFinishing = true;
			}

			if (subwayTrain.x < -4000 && subwayFinishing)
				subwayReset();
		}
	}
	...
	function subwayReset():Void
		{
			...
			// trainSound.time = 0;
			subwayCars = 8;
			subwayFinishing = false;
			startedMovingSubway = false;
		}
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			case 'subway':
				...
				if (curBeat % 8 == 4 && FlxG.random.bool(30) && !subwayMoving && subwayCooldown > 8)
				{
					subwayCooldown = FlxG.random.int(-4, 0);
					subwayStart();
				}
		}
