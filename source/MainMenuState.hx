...
class MainMenuState extends MusicBeatState
{
	...
	var optionShit:Array<String> = ['storymode', 'freeplay', #if ACHIEVEMENTS_ALLOWED 'awards', #end 'credits', #if !switch 'donate', #end 'options'];

	var magenta:FlxSprite;
	var checker:FlxBackdrop = new FlxBackdrop(Paths.image('Main_CheckerSTORY'), 0.5, 0.5, true, true);
	...
	override function create()
	{
		...
		var bg:FlxSprite = new FlxSprite(0).loadGraphic(Paths.image('bgwfilter'));
		bg.screenCenter();
		...
		var bgglow:FlxSprite = new FlxSprite(0).loadGraphic(Paths.image('bgglow'));
		bgglow.screenCenter();
		bgglow.antialiasing = ClientPrefs.globalAntialiasing;
		add(bgglow);

		var buttons:FlxSprite = new FlxSprite(80).loadGraphic(Paths.image('buttonsthing'));
		...
		{
			var offset:Float = 203 - (Math.max(optionShit.length, 4) - 4) * 120;
			...
			menuItem.setGraphicSize(Std.int(menuItem.width * 0.31));
		...
		add(versionShit);
		var versionShit:FlxText = new FlxText(12, FlxG.height - 24, 0, "FNF - Rising Volume v" + Application.current.meta.get('version'), 12);
		versionShit.scrollFactor.set();
		versionShit.setFormat("VCR OSD Mono", 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(versionShit);
